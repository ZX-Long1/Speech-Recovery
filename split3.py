...
def train(classifier, inversion, device, data_loader, optimizer, epoch, split_point, nTV, noise):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            noise_tensor = torch.normal(0, 0.1 * noise, prediction.shape).to(device)
            prediction = prediction + noise_tensor
...
def test(classifier, inversion, device, data_loader, split_point, target_dir, noise, epoch):
    classifier.eval()
    inversion.eval()
    mse_loss = 0
    with torch.no_grad():
...
def main():
    ...
    for i in range(0, 3):
        ...
        target_dir = '../Speech_Command/Result/noise_mel_spect_inversion/split' + str(
            args.split_point) + '_noise' + str(args.noise) + '/'
        ...
        for epoch in range(1, args.epochs + 1):
            train(classifier, inversion, device, train_loader, optimizer, epoch, args.split_point, args.nTV,
                  args.noise)
            cl_acc = test(classifier, inversion, device, test_loader, args.split_point, target_dir, args.noise, epoch)
