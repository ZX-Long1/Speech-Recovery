...
def mfcc_mse_plot(data_loader, args, device, cuda, target_dir):
    ...
    for j in [0, 5, 10, 20,100]:
        ...
        for i in range(4):  # 代表四个分割点
            ...
            # 替换！！！！！！！！！！
            inversion_path = '../SplitLearn/UrbanSound8K/mfcc/blackbox_inversion/split_point' + str(i + 1) + '/inversion_' + str(j) + '/train_record/inversion.pth'
            try:
...
def main():
    ...
    # mfcc
    mfcc_acc_100_set = StudioSet(root="../UrbanSound8K/UrbanSound8K/audio/mfccs_acc_100", transform=transform)
    ...
    # 绘制mse图像
    mfcc_mse_100_target_dir ='../SplitLearn/UrbanSound8K/Result/mse/mfcc_blackbox/'
    mfcc_mse_plot(mfcc_acc_100_loader,args,device,cuda,mfcc_mse_100_target_dir)

    # # 绘制snr图像
    mfcc_snr_100_target_dir = '../SplitLearn/UrbanSound8K/Result/snr/mfcc_blackbox/'
