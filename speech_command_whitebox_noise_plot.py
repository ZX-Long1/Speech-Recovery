...
parser.add_argument('--noise', type=int, default=0, metavar='')

def noise_acc(data_loader,args,device,cuda,target_dir):
    print(target_dir)
    ...
    inversion_path = '../Speech_Command/Result/noise_mel_spect_inversion/split' + str(args.split_point) +'_noise'+str(args.noise) +'/train_record/inversion.pth'
...
def noise_SNR(data_loader,args,device,cuda,target_dir):
    print(target_dir)
    classifier = ResNet18(BasicBlock=BasicBlock, nc=args.nc, nz=args.nz).to(device)
    classfier_path = '../Speech_Command/Result/classifier/mel_spect_train_record/classifier.pth'
    str1 = 'epoch'
...
def noise_mse_plot(data_loader,args,device,cuda,target_dir):
    print(target_dir)
    classifier = ResNet18(BasicBlock=BasicBlock, nc=args.nc, nz=args.nz).to(device)
    classfier_path='../Speech_Command/Result/classifier/mel_spect_train_record/classifier.pth'
    str1 = 'epoch'
    ...
    #         for data, target in data_loader:
    #             data, target = data.to(device), target.to(device)
    #             split_data = classifier(data, args.split_point,args.split_point,args.noise)
    #             reconstruction = inversion(split_data)
    #
    ...
    # mse_dir = '../mse_plot/noise_split'+str(args.split_point)+'.npy'
    ...
    mse_dir = '../snr_plot/noise_split'+str(args.split_point)+'.npy'
    np.save(mse_dir, record_mse)
...
def main():
    ...
    args.split_point = 1
    noise_mse_plot(mel_spect_acc_100_loader, args, device, cuda, '')
    args.split_point = 3
    noise_mse_plot(mel_spect_acc_100_loader, args, device, cuda, '')

    # os.makedirs('../SNR/SpeechCommand_noise/', exist_ok=True)
    # os.makedirs('../acc/SpeechCommand_noise/', exist_ok=True)
    # noise = [0, 1, 5, 10, 15, 20]
    # split=[1,3]
    # for j in range(0, 2):
    #     for i in range(0, 6):
    ...
    #
    #         # # 计算SNR
    #         # mel_spect_acc_100_target_dir = '../SNR/SpeechCommand_noise/split' + str(args.split_point) + '_noise'+str(args.noise)+'_100.txt'
    #         # mel_spect_acc_all_target_dir = '../SNR/SpeechCommand_noise/split' + str(args.split_point) + '_noise'+str(args.noise)+'_all.txt'
    #         # noise_SNR(mel_spect_acc_100_loader, args, device, cuda, mel_spect_acc_100_target_dir)
    #         # noise_SNR(mel_spect_acc_all_loader, args, device, cuda, mel_spect_acc_all_target_dir)
    #
    #         #计算acc
    #         mel_spect_acc_100_target_dir = '../acc/SpeechCommand_noise/split' + str(args.split_point) + '_noise' + str(args.noise) + '_100.txt'
    #         mel_spect_acc_all_target_dir = '../acc/SpeechCommand_noise/split' + str(args.split_point) + '_noise' + str(args.noise) + '_all.txt'
    #         noise_acc(mel_spect_acc_100_loader, args, device, cuda, mel_spect_acc_100_target_dir)
    #         noise_acc(mel_spect_acc_all_loader, args, device, cuda,mel_spect_acc_all_target_dir)
