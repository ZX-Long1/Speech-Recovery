#Mel_spect表示  whitebow_attack  训练
...
from Segment_dataset import StudioSet       #数据集
from Dataset1 import StudioSet as StudioSet2
...
parser.add_argument('--cuda_id', type=int, default=3, metavar='')
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        if batch_idx % 50 == 0:
            print('Train Epoch: {} [{}/{}]\tLoss: {:.8f}'.format(epoch, batch_idx * len(data),
                                                                 len(data_loader.dataset), loss_TV.item()))
...
def record(classifier, inversion, device, data_loader, epoch, num, target_dir, loss, split_point):
    ...
    with (torch.no_grad()):
        for data, target in data_loader:
            ...
            for i in range(num):  # num=24
                # 原始图像
                # 将张量从 CUDA 移动到 CPU
                mel_spect1 = truth[i].cpu()
                # 去掉第一个维度
                mel_spect1 = mel_spect1.squeeze(0)
                # print(mfccs1.shape)

                # 转换为 NumPy 数组
                mel_spect1_numpy = mel_spect1.numpy()
                # print(mfccs_numpy1.shape)

                # 恢复原始音频参数
                mel_spect1_numpy = mel_spect1_numpy * ((max_val - min_val)) + min_val

                # 可视化mel_spect
                sr = 25250
                plt.figure(figsize=(8, 4))
                librosa.display.specshow(mel_spect1_numpy, sr=sr, x_axis='time', y_axis='mel', hop_length=512,
                                         cmap='magma')
                ...
                # 反转生成图像
                mel_spect2 = inverse[i].cpu()
                mel_spect2 = mel_spect2.squeeze(0)
                mel_spect2_numpy = mel_spect2.numpy()
                mel_spect2_numpy = mel_spect2_numpy * ((max_val - min_val)) + min_val

                sr = 25250
                plt.figure(figsize=(8, 4))
                librosa.display.specshow(mel_spect2_numpy, sr=sr, x_axis='time', y_axis='mel', hop_length=512,
                                         cmap='magma')
                plt.ylabel('Mel Frequency')
                plt.xlabel('Time (s)')
                plt.title(f'Mel Spectrogram')
    ...
    #         inverse = reconstruction[0:num]
            ...
            #     sr = 25250
            #     plt.figure(figsize=(8, 4))
            #     librosa.display.specshow(mel_spect1_numpy, sr=sr, x_axis='time', y_axis='mel',hop_length=512, cmap='magma')
            #     plt.ylabel('Mel Frequency')
            #     plt.xlabel('Time (s)')
            #     plt.title(f'Mel Spectrogram')
            #     plt.colorbar(format='%+2.0f dB')  # 添加色条，显示dB值
            ...
            #     sr = 25250
            #     plt.figure(figsize=(8, 4))
            #     librosa.display.specshow(mel_spect2_numpy, sr=sr, x_axis='time', y_axis='mel', hop_length=512,cmap='magma')
            #     plt.ylabel('Mel Frequency')
            #     plt.xlabel('Time (s)')
            #     plt.title(f'Mel Spectrogram')
            #     plt.colorbar(format='%+2.0f dB')  # 添加色条，显示dB值
...
def main():
    ...
    target_dir_root = '../SegmentLearn/mel_spect/blackbox_inversion/split_point'
    ...
    test_set2 = StudioSet2(root="../UrbanSound8K/UrbanSound8K/audio/mel_spect_acc_100", transform=transform)
    test_loader2 = torch.utils.data.DataLoader(test_set2, batch_size=32, shuffle=False, **kwargs)
    ...
    # for i in [1]:
        ...
        path = '../SplitLearn/UrbanSound8K/mel_spect/classifier/classifier_100' + '.pth'
        str1 = 'epoch'
        str2 = 'acc'
        print(path)
        try:
        ...
        # optimizer = optim.Adam(inversion.parameters(), lr=0.0005, betas=(0.5, 0.999), amsgrad=True)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if cl_acc < best_acc:
                ...
                record(classifier, inversion, device, test_loader2, epoch, 24, target_dir, cl_acc, args.split_point)
