...
def train(classifier, inversion, loader, optimizer, epoch, sp, nTV, noise_std, log_interval, device):
    classifier.eval()
    inversion.train()
    for batch_idx, (data, target) in enumerate(loader):
        ...
        recon_pred = classifier(reconstruction + noise, split_point=sp)
...
def main():
    ...
    wb_dir = 'whitebox_shallow_robust' if args.shallow else 'whitebox'
    ...
    for epoch in range(1, args.epochs + 1):
        train(classifier, inversion, train_loader, optimizer, epoch, args.split_point, args.nTV, args.noise_std, args.log_interval, device)
