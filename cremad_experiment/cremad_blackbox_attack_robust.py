...
def train(classifier, inversion, loader, optimizer, epoch, sp, nTV, noise_std, log_interval, device, feat):
    classifier.eval()
    inversion.train()
    step = 0.001 if feat == 'mel' else 0.01
    for batch_idx, (data, target) in enumerate(loader):
        ...
        # Fixed noise for this batch (detached, not part of computation graph)
        noise = torch.randn_like(reconstruction) * noise_std
        ...
        with torch.no_grad():
            ...
            for _ in range(1, 50):
                ...
                # NES sampling around NOISY reconstruction
                p1 = reconstruction + noise + step * rand_dir
                p2 = reconstruction + noise - step * rand_dir
...
def main():
    ...
    print(f'Device: {device}  feat={args.feat}  sp={args.split_point}  noise_std={args.noise_std}')
    ...
    bb_dir = 'blackbox_shallow_robust' if args.shallow else 'blackbox'
    out_dir = f'/media/sda1/zxlong/tmc_re/CREMA-D/inversion/{args.feat}/{bb_dir}/split_point{args.split_point}/'
    ...
    for epoch in range(1, args.epochs + 1):
        train(classifier, inversion, train_loader, optimizer, epoch, args.split_point, args.nTV, args.noise_std, args.log_interval, device, args.feat)
