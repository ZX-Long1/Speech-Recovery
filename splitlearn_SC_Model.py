...
class Conv_sp3(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(256, 128, kernel_size=3, stride=1, padding=1),  # 32x48
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),

            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),  # 32x48-> 64x96
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),

            nn.ConvTranspose2d(64, 1, kernel_size=3, stride=1, padding=1),  # 64x96
            nn.Sigmoid()
        )
...
class Conv_sp4(nn.Module):
    def __init__(self):
        ...
        #     nn.ReLU(inplace=True),
        #
        #     nn.ConvTranspose2d(512, 256, kernel_size=4, stride=2, padding=1),  # 16*24 -> 32x48
        #     nn.BatchNorm2d(256),
        #     nn.ReLU(inplace=True),
        #
        #     nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1),  # 32x48 -> 64*96
        #     nn.BatchNorm2d(128),
        #     nn.ReLU(inplace=True),
        #
        #     nn.ConvTranspose2d(128, 64, kernel_size=3, stride=1, padding=1),  # 64x96
        #     nn.BatchNorm2d(64),
        #     nn.ReLU(inplace=True),
        #
        #     nn.ConvTranspose2d(64, 1, kernel_size=3, stride=1, padding=1),  # 64x96
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(256, 256, kernel_size=4, stride=2, padding=1),  # 4x6 -> 8x12
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),

            nn.ConvTranspose2d(256, 256, kernel_size=4, stride=2, padding=1),  # 8x12 -> 16x24
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),

            nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1),  # 16x24 -> 32x48
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            ...
            nn.ConvTranspose2d(64, 32, kernel_size=3, stride=1, padding=1),  # 64x96
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),

            nn.ConvTranspose2d(32, 1, kernel_size=3, stride=1, padding=1),  # 64x96
            nn.Sigmoid()
        )
