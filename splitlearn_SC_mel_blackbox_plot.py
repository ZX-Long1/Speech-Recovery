...
parser.add_argument('--num_workers', type=int, default=20, metavar='')
...
def mel_spect_mse_plot(data_loader, args, device, cuda, target_dir):
    ...
    for j in [0, 5, 10, 20,100]:
        ...
        for i in range(4):  # 代表四个分割点
            ...
            # 替换！！！！！！！！！！
            inversion_path = '../SplitLearn/Speech_Command/mel_spect/blackbox_inversion/split_point' + str(i + 1) + '/inversion_' + str(j) + '/train_record/inversion.pth'
            try:
                checkpoint = torch.load(inversion_path, map_location={'cuda:' + str(args.cuda_id): cuda})
                inversion.load_state_dict(checkpoint['model'])
...
def main():
    ...
    # # 绘制mse图像
    mel_spect_mse_100_target_dir ='../SplitLearn/Speech_Command/Result/mse/mel_blackbox/'
    mel_spect_mse_plot(mel_spect_acc_100_loader,args,device,cuda,mel_spect_mse_100_target_dir)

    # # 绘制snr图像
    mel_spect_snr_100_target_dir = '../SplitLearn/Speech_Command/Result/snr/mel_blackbox/'
