...
def mfcc_mse_plot(data_loader, args, device, cuda, target_dir):
    ...
    for j in [0, 5, 10, 20,100]:
        ...
        for i in range(4):  # 代表四个分割点
            ...
            inversion_path = '../SplitLearn/Speech_Command/mfcc/whitebox_inversion/split_point' + str(i + 1) + '/inversion_' + str(j) + '/train_record/inversion.pth'
            try:
                checkpoint = torch.load(inversion_path, map_location={'cuda:' + str(args.cuda_id): cuda})
                inversion.load_state_dict(checkpoint['model'])
...
def main():
    ...
    # # # 绘制mse图像
    # mfcc_mse_100_target_dir ='../SplitLearn/Speech_Command/Result/mse/mfcc_whitebox/'
    # mfcc_mse_plot(mfcc_acc_100_loader,args,device,cuda,mfcc_mse_100_target_dir)

    # # 绘制snr图像
    mfcc_snr_100_target_dir = '../SplitLearn/Speech_Command/Result/snr/mfcc_whitebox/'
