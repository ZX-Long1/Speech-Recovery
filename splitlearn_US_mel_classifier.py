...
from Dataset1 import StudioSet
from splitlearn_US_Model import Classifier,Conv_sp1,Conv_sp2,Conv_sp3,Conv_sp4
import numpy as np
import random
...
parser.add_argument('--cuda_id', type=int, default=0, metavar='')
...
def main():
    ...
    # target_dir="../UrbanSound8K/UrbanSound8K/classifier/mel_spect_train_record/"
    target_dir = "../SplitLearn/UrbanSound8K/mel_spect/classifier/"
    ...
    use_cuda = not args.no_cuda and torch.cuda.is_available()  # 判断是否使用cuda 根据是否使用CUDA（GPU加速）和CUDA是否可用，设置 use_cuda 变量
    device = torch.device(cuda if use_cuda else "cpu")  # 设置设备
    torch.cuda.set_device(args.cuda_id)  # 设置cuda设备
    # kwargs = {'num_workers': args.num_workers,
    #           'pin_memory': False} if use_cuda else {}  # 设置数据加载的参数 如果使用cuda则设置工作进程数和是否使用pin_memory 为False
    ...
    train_set = StudioSet(root='../UrbanSound8K/UrbanSound8K/audio/train_set_mel_spect_numpy', transform=transform)
    test_set = StudioSet(root='../UrbanSound8K/UrbanSound8K/audio/test_set_mel_spect_numpy', transform=transform)
    ...
    classifier = Classifier(nc=args.nc, ndf=args.ndf, nz=args.nz, select_layer=0, frac=0).to(device)
